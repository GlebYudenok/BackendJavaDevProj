...
/*
2 ошибки:
    1. Поле MESSAGE_CVV_PREFIX имело строковый литерал, символы которого были написаны на разных языках.
    2. В xml документе был тэг <Message> что вызывало метод maskCipherCVCInMsg, но внутри тэга не было префикса,
    по которому определялось бы место вставки '---', поэтому, так как префикс не был найден,
    функция вернула -1, но выход из основного метода не был выполнен, что продолжило работу и привело к ошибке.
 */
